...
This file defines the repository-specific development rules and the operating boundaries for AI agents working on `longitude-one/spatial-types`.

## Project Structure & Module Organization

This PHP library implements spatial interfaces and immutable geometric and geographic value types, with factories, spatial references, and validation.

Source lives in `lib/`, using Composer PSR-4 autoloading for `LongitudeOne\SpatialTypes\`. Public contracts live in `Interfaces/`, concrete spatial classes in `Types/`, construction helpers in `Factory/`, spatial references in `Reference/`, coordinate values in `Value/`, and validation in `Validator/`.

Spatial values are immutable through the public API. Preserve the documented constructor and `with*` contracts. Changing the declared spatial reference does not transform coordinates.

Tests use the `LongitudeOne\SpatialTypes\Tests\` namespace under `tests/`. Unit tests mirror the source under `tests/Unit/`; functional tests live in `tests/Functional/`, and issue regressions in `tests/Issue/`.
...
Read [`docs/instantiable-spatial-types.md`](docs/instantiable-spatial-types.md) and [`docs/type-class-hierarchy.md`](docs/type-class-hierarchy.md) for supported types and their public contracts. Consult [`docs/immutability.md`](docs/immutability.md), [`docs/spatial-reference-systems.md`](docs/spatial-reference-systems.md), and [`docs/validators.md`](docs/validators.md) when changing the corresponding behavior.
...
- Run `composer install --working-dir=quality/<tool>` for each of `php-cs-fixer`, `php-stan`, `php-mess-detector`, and `php-code-sniffer` to install the four quality tool environments.
- `composer test`: run PHPUnit.
- `vendor/bin/phpunit --no-coverage tests/Unit/Types/Dimension2/Geography/PointTest.php`: run focused tests without coverage.
- `composer test-local`: run tests and print the coverage summary; enable PCOV first (required by `require-dev`). Reports go to `.phpunit-cache/`.
...
Concrete types live under `LongitudeOne\SpatialTypes\Types`, with `Dimension2` (XY), `Dimension3z` (XYZ), `Dimension3m` (XYM), and `Dimension4zm` (XYZM) namespaces, each containing `Geometry` and `Geography` families.

Pass spatial references and collection elements through the supported constructors or factories, preserving the documented family, dimension, and reference compatibility rules.
...
Assert coordinate values, concrete types, dimensions, spatial references, and validation failures as applicable. For immutable replacement operations, verify that the original value remains unchanged.
...
```text
feat: add immutable point replacement
fix: preserve spatial reference in empty geometry collections
perf: reduce allocations during coordinate validation
refactor: extract coordinate validation
docs: document spatial reference handling
test: add XYZM point replacement cases
...
```text
feat(types): add immutable point replacement
fix(factory): preserve spatial reference in collections
test(types): add empty geometry cases
```
...
```text
feat!: change public factory API
```
...
BREAKING CHANGE: SpatialFactory construction now requires an explicit registry.
```
