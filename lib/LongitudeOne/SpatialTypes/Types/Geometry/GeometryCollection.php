...
class GeometryCollection extends AbstractSpatialType implements CollectionInterface
{
    /**
    ...
    {
        if ($spatial instanceof GeometryCollection) {
            throw new InvalidValueException('GeometryCollection cannot contain another GeometryCollection.');
        }

        if (!$this->hasSameDimension($spatial)) {
            throw new InvalidDimensionException('GeometryCollection cannot contain elements with different dimensions.');
        }

        if ($this->getFamily() !== $spatial->getFamily()) {
            throw new InvalidFamilyException('GeometryCollection cannot contain elements with different families.');
        }

        if (!empty($spatial->getSrid()) && !empty($this->getSrid()) && $this->getSrid() !== $spatial->getSrid()) {
            throw new InvalidSridException('GeometryCollection cannot contain elements with different SRIDs.');
        }
